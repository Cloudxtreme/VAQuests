...
sub EVENT_SAY {
   if ($text=~/hail/i && defined($qglobals{MMCtalk}) && plugin::check_hasitem($client, 41000)) {
      ...
      if($mmc_wins >= 52 && $qglobals{MMCtalk} == 25) {
         ...
         plugin::DiaWind("Shhh, keep it down. There are many who would strongly disapprove of my protection of one of the vampires from Mistmoore's Catacombs. I've been keeping him safely hidden, so do not expect or attempt to usurp my efforts to learn ways to protect ourselves from the evil that lurks in those caves. There is much to learn about the Progeny. We know very little about them, other than they are the elite vampires. Some of them are even considered Mayong Mistmoore's children.");
         ...
         plugin::DiaWind("Ah yes. Have you encountered Mayong Mistmoore's children yet? They call themselves the Progeny. But, there is some dispute about which of them should be the ruler of the catacombs. As a result, Mistmoore's children have aligned themselves in two ways: the Trueborn and the Scions.");
         $client->Message(15, "You have received an invaluable piece of information!");
         if ($mmc_wins >= 56) {
            $client->Message(13, "You have additional information to uncover! Try speaking to this adventurer again."); # red message
         }
         ...
         plugin::DiaWind("I'm afraid I can tell you no more. The source of my information has diappeared. I'm not sure if he was found, or if he left of his own accord. During all this time though, Vual Stoutest was able to create another Wayfarers Brotherhood party to get to the still heart of the catacombs. You should go talk to him. Be well traveler. The world is becoming stranger by the hour.");
         $client->Message(15, "You have received an invaluable piece of information!");
