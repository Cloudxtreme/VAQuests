sub EVENT_SAY {
	if($text=~/Hail/i){
		plugin::DiaWind("Yes. yes!!  What do I have here?!!  Another [new recruit]?  If so. then speak up!  If not. then leave and do not waste my time nor risk your life.  I also seek a [legion soldier] if you be one.");
	}
	if((($text=~/new recruit/i) || ($text=~/legion soldier/i) || ($text=~/ready for my task/i)) && ($faction > 4)) {
		plugin::DiaWind("No Iksar resident will have anything to do with you!!");
	}
	if(($text=~/new recruit/i) && ($faction < 5)) {
		plugin::DiaWind("Yes.  You have the look of the Partisan.  I trust you have begun your blacksmith training.  If not. then do so.  Also. you should read all the books available to you in Fortress Talishan.  We are not dimwitted broodlings here.  You shall need the knowledge soon. That. or a coffin.  Ha!!  Here is your task. are you [ready for your task]?");
	}
	if(($text=~/legion soldier/i) && ($faction < 5)) {
		plugin::DiaWind("Good news to my ears!!  I seek to prove to the War Baron that the infamous forsaken band of thieves who call themselves Marrtail's Marauders are operating within earshot of our city.  You must bring me proof that you encountered no fewer than four of these thieves.  Do so and I shall offer you an armor item unavailable to most.");
	}
	if(($text=~/ready for my task/i) && ($faction < 5)) {
		plugin::DiaWind("Yes. yes!!  It does not matter.  You must be ready.  I will hand you the Partisan pack.  Into it you shall combine one giant blood sac of the giant leech;  scout beads from a goblin scout; one sabertooth kitten canine and finally. three bone shards from decaying skeletons.  Hopefully. you will survive your attempt to obtain these items.  Return the full Partisan pack and you shall be rewarded with a curscale shield.");
...
sub EVENT_ITEM {
	if($faction < 5) {
		...
		if (plugin::check_handin(\%itemcount, 12673 => 1)) {
			plugin::DiaWind("Well done recruit, I knew we could count on you to do the job.  Here is the reward you were promised");
