...
sub EVENT_SAY {
  if (($text=~/hail/i) && ($event == 1)) {
    plugin::DiaWind("Don't bother me until your work is done!");
  }
  ...
  if (($text=~/hail/i) && ($event == 0)) {
    plugin::DiaWind("Find the aged stonemites. They are causing great trouble, come back and talk to me once you have solved the problem.");
