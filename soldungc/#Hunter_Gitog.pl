...
sub EVENT_SAY {
  ...
  if($text=~/fire goblins/i) {
    plugin::DiaWind("They were once members of the Taklasinai fire goblin tribe that lair further within the caverns. However they fed upon the flesh of the spiders that live in the caves. The spider flesh is poisonous. It burns the brain causing madness and rage. These fire goblins are out of control. They attack any who venture near them. They have recently attacked several members of our tribe in our hunting grounds. If you [assist] us in stopping them, you will be rewarded.");
  }
  if($text=~/assist/i) {
    plugin::DiaWind("I will reward you for every four brain-scorched goblin heads you bring to me.");
  }
}
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 55252 => 4)) {#Brain-Scorched Goblin Heads
    plugin::DiaWind("These will be no more threat.  Take this, $name.");#text made up
