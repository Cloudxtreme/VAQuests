sub EVENT_SAY {
  ...
  if ($text=~/the scout was faulty/i) {
    if (defined($qglobals{bic}) && ($qglobals{bic} >= 11) && ($qglobals{bic_sew} == 12) && ($qglobals{bic_vxe} == 13) && ($qglobals{bic_tip} == 14)) {
      plugin::DiaWind("Oh dear, a faulty scout? I do hope you followed the instructions properly! Never mind, here are some replacement parts to try again with.");
...
sub EVENT_ITEM {
  ...
  if (plugin::check_handin(\%itemcount, 67554 => 1)) {
    if (defined($qglobals{bic}) && ($qglobals{bic} >= 11)) {
      ...
      plugin::DiaWind("You have not completed all of the areas needed to progress, please return when you have done so.");
