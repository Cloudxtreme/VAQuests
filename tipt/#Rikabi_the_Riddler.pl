...
sub EVENT_SAY {
  ...
  if (($text=~/ready/i) && ($event == 0)) {
    ...
    if ($question == 1) {
      plugin::DiaWind("Who makes has no need of it, who buys has no use of it, who uses it can neither see nor feel it.");
    }
    if ($question == 2) {
      plugin::DiaWind("What can be held in the left hand, but not in the right hand.");
    }
    if ($question == 3) {
      plugin::DiaWind("I drive men mad for love of me, Easily beaten, Never free.");
    }
    if ($question == 4) {
      plugin::DiaWind("I'm always hungry, I must always be fed, The finger I lick will soon turn red.");
    }
    if ($question == 5) {
      plugin::DiaWind("My life is measured in hours. I serve by being devoured. Thin, I am quick, Fat I am slow. Wind is my foe.");
    }
    if ($question == 6) {
      plugin::DiaWind("My mouth bears whiskers but no teeth. I have scales but I do not weigh. You can bait me with delicious food but please, no drink today.");
    }
    if ($question == 7) {
      plugin::DiaWind("Lovely and round, I shine with pale light, grown in darkness, a lady's delight.");
    }
    if ($question == 8) {
      plugin::DiaWind("I'm offered to the loved, and also to the dead. I come in many varied hues, most notably red. My pricks are known to pierce the skin, often resulting in wounds that bled.");
    }
    if ($question == 9) {
      plugin::DiaWind("When I am filled I can point the way, When I am empty Nothing moves me, I have two skins One without and one within.");
    }
    if ($question == 10) {
      plugin::DiaWind("I help to mature your spirits. When moistened I fulfill my purpose. Should I dry out, my task will fail and my quarry may be worthless.");
    }
    if ($question == 11) {
      plugin::DiaWind("I build up castles. I tear down mountains. I make some men blind, I help others to see. What am I?");
    }
    if ($question == 12) {
      plugin::DiaWind("When young, I am sweet in the sun. When middle-aged, I make you happy. When old, I am valued more than ever.");
    }
    if ($question == 13) {
      plugin::DiaWind("I scribble forms of the finest letter, and repel elements of the harshest weather. I am an arrow-aimer and a dust-breaker.");
    }
    if ($question == 14) {
      plugin::DiaWind("Searing 'cross the pitch-black skies, I scream in celebration, yet moments later, my outburst through, I am naught but imagination.");
    }
    if ($question == 15) {
      plugin::DiaWind("It floats on water, light as can be. A thousand men can't lift it free.");
    }
  }
}
...
sub EVENT_ITEM {
  if (($question == 1) && plugin::check_handin(\%itemcount,  54068 => 1)) { #Coffin
    ...
    plugin::DiaWind("That is incorrect.");
  ...
  if ($event == 2) {
    plugin::DiaWind("You hear the grinding of stone as the door unlocks before you.");
    $entity_list->FindDoor(4)->SetLockPick(0);
    if (defined($qglobals{bic}) && ($qglobals{bic} == 11)) {
	  plugin::DiaWind("'I sense you have come here seeking something besides passage. Please take this it was left here by one of your people's constructs.");
