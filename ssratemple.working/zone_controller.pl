...
sub EVENT_SPAWN {
  ...
  quest::settimer("EmpCycle",1); #Cyclical Timer
}
...
#    if ($check == 0 && defined $qglobals{cursed_dead}) {
  ...
  if ($timer eq "EmpCycle") {
    ...
    if (!defined $qglobals{"Emp1"} && !defined $qglobals{"Emp2"} && !defined $qglobals{"Emp3"} && !defined $qglobals{"BloodCoolDown"}) { #Emperor is ready to spawn
      quest::setglobal("Emp1",1,3,"H6"); #Normal Cycle Start
    }
    # The Blood lockout has passed, spawn him.
    if (defined $qglobals{"Emp1"} && !$entity_list->GetNPCByNPCTypeID(162064) && !defined $qglobals{"BloodDead"} && !$entity_list->GetNPCByNPCTypeID(162189) && ($EmpPrep == 0)) {
      quest::spawn2(162189,0,0,877.0,-325.0,400.5,192.0); ##Blood_of_Ssraeshza
    }
    # Blood is still locked out, spawn the Blood Golem.
    if (!defined($qglobals{"BloodCoolDown"}) && defined $qglobals{"Emp1"} && !$entity_list->GetNPCByNPCTypeID(162064) && !$entity_list->GetNPCByNPCTypeID(162189) && !$entity_list->GetNPCByNPCTypeID(162227) && ($EmpPrep == 0)) {
      quest::spawn2(162064,0,0,877.0,-325.0,400.5,192.0); #Ssraeshzian_Blood_Golem
    }
    # Emperor timer is expired, go ahead and spawn him.
    if (!$entity_list->GetNPCByNPCTypeID(162065) && !defined $qglobals{"Emp3"}) {
...
#    quest::setglobal("Emperor",2,0,"F");
    quest::delglobal("Emp1");
    quest::setglobal("Emp2",1,3,"6H");
    $EmpPrep = 0;
  }
}
...
sub EVENT_SIGNAL {
  ...
  if ($signal == 5) { #Emperor is dead
    quest::delglobal("Emp2");
    quest::setglobal("Emp3",1,3,"M$EmpRepopTime"); #Emp respawn timer
  }
