sub EVENT_SAY {
 if($text =~ /Hail/i) {
	plugin::DiaWind("Hello there traveler. I have many wares for sale so please take a look at what I have. If you think all my goods are junk and wish to buy from someone else though I'll understand.");
 }
}
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 26057 => 1)) {#Book of Sales Secrets
    plugin::DiaWind("'I see you have brought a book from the Commissioner. Well I'm not much for reading but I will surely try anything that may increase my sales. Anyways I'm sure you arent that interested in how bad my sales are and would like to be on your way so here you go.");
