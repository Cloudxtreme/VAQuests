...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 20612 => 1)) { #Piece of the Staff
    if(defined($qglobals{NecroPre}) && ($qglobals{NecroPre} == 1)) {
      plugin::DiaWind("I ask you to find a staff of great power and you bring me this??! A piece of the staff? You are an incompetent fool! Take this trash back and give it to Kazen so that he may punish you as he sees fit! Begone!");
      quest::summonitem(20612); #Piece of the Staff
      quest::setglobal("NecroPre",2,5,"F");
    }
    else {
      plugin::DiaWind("What is this trash you give me?"); #Text made up
