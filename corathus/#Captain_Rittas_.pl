sub EVENT_SAY
{
	...
	if($boss_check)
	{
		...
		elsif($text=~ /Assignment/i)
		{
			...
			{
				plugin::DiaWind("These assignments are all military in nature, please return to me if you succeed in finishing one.");
				quest::taskselector(@task_array);
			}
			else
			{
				plugin::DiaWind("I have nothing to offer you currently.");
			}
		}
	}
}