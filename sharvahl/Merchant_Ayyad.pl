sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::DiaWind("Hello $name. Sorry but I have no time to chitchat. I’m looking for the help of a citizen.");
  }
  ...
  if ($text=~/additional instructions/i) {
    plugin::DiaWind("Mastered those runes already, $name? You're learning very fast indeed. Your next lesson will be in the molding of a new type of item. You'll need to gather some gray mud from below the city and mix it with some Rockhopper blood to make a block of reddish clay. Take this block of reddish clay and combine it with a Vah Shir model sketch to create an unfired Vah Shir figurines. Fire the model in a kiln with a firing sheet to create a Finished Vah Shir figurine. Finally, use the Runequill Set to etch grimling blood into runes on the figurine.");
  }
}
...
sub EVENT_ITEM {
  ...
  if (plugin::check_handin(\%itemcount, 3498 => 1)) { #Gray Jar
    plugin::DiaWind("Excellent! Please accept this knapsack as payment for your trouble. Here is my seal as well. I can always use more jars and if you give me four of my seals I will share with you a family secret.");
  ...
  if (($base_skill >= 105) && plugin::check_handin(\%itemcount, 3631 => 1)) { #Pottery skill at least 105, Basic Runequill Set
    plugin::DiaWind("You're progressing nicely, $name. Take this sculpting tool and add it to the set you already have. This new quill set will be able to make some very nice urns that should help ease the burden of moving all that heavy clay around. Take a gray jar and use the new set of tools I've given you to etch Xakra blood into runes on the side of the jars. These should prove to be very useful in your labors in the future. Once you have mastered those runes, bring back your runequill set to me for another addition.");
