...
sub EVENT_SAY {
if ($text=~/hail/i) {
plugin::DiaWind("[Yes] or [No]. [Break]");
}
...
plugin::DiaWind( "quest::spawn2(ReplaceID,0,0, $LocX, $LocY, $LocZ , $LocH);") ;
...
$rndadd = $rndadd + 1;
#plugin::DiaWind( "quest::spawn2(ReplaceID,0,0, $LocX, $LocY, $LocZ , $LocH);") ;
plugin::DiaWind( "if (rand == $rndadd){quest::spawn2(298200,0,0, $LocX, $LocY, $LocZ , $LocH); }") ;
