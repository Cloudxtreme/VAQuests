...
if (plugin::check_handin(\%itemcount, 5954 =>1 )) {
  plugin::DiaWind("Ah so Gawfed wants his vase.  Unfortunately that vase has been broken.  Maybe you could go and speak with Chogar so that he can repair the vase.  Then Gawfed and I can finish our transaction.");  #ad lib text
