...
sub EVENT_SAY {
  if(defined($qglobals{AidEinoDone}) && ($qglobals{AidEinoDone} == 1)) { #Can't do the turn in without getting the bowl, so I'm not worried about restricting the turn ins on the flag, just this text and getting the bowl.
    if($text=~/hail/i) {
      plugin::DiaWind("Greetings $name! It pleases me very much that you were able to help Eino out in his time of need.' Councilwoman Kerasha glances at Eino, 'Tunare only knows what I would do without him at my side. There is another matter at hand that has come to the attention of the council that is beyond even our combined power and requires the aid of a powerful alliance. Would you be willing to aid Tanaan?");
    }
    if($text=~/aid/i) {
      plugin::DiaWind("A rift in this plane at the point of harmony upon which the city of Tanaan is built is crumbling. The other council members do not see it happening for they lack the sight that I possess. Granted it will take time but it is something that must be set to right or our home will fall to waste among the tides of the Planes.' Councilwoman Kerasha's body grows rigid. 'I did not spend my life in defense of this city to watch it crumble to dust! The way to repair the breach is difficult but I feel Tunare will grant me the strength once I have the four essences of power in my grasp.");
    }
    if($text=~/essences of power/i) {
      plugin::DiaWind("The powers that watch over the four elemental planes all guard their respective essence of power. You must defeat each of them in turn and place them all within this sacred bowl. Once I have the combined power of the planes I can begin the ceremony and set the foundation of the city to rights. Thank you for your offer to assist me $name.");
      quest::summonitem(17183); #Sacred Bowl
    }
  }
  else {
    if($text=~/hail/i) {
      plugin::DiaWind("Greetings $name. I am Councilwoman Kerasha, representative of the Selia district. I provide the voice of nature to the Council of Tanaan. Woe is the city in any realm that forgets to heed Mother Tunare. For where else could one draw strength from but the land?");
    }
  }
}
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 16266 => 1)) { #Power of the Planes
    ...
    plugin::DiaWind("I understand. Take this, may it serve you well, $name.");
