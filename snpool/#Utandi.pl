...
sub EVENT_SAY {
  if (defined($qglobals{sewers}) && ($qglobals{sewers} == 5) && ($event == 0)) {
    if ($text=~/hail/i) {
      plugin::DiaWind("I have a problem.");
    }
    if ($text=~/problem/i) {
      plugin::DiaWind("I have lost my map. It shows me how to get home. Can you help me find it?");
    }
    if ($text=~/i will help/i) {
      plugin::DiaWind("Thanks! Try to find a big pile of slime, I think he has my map.");
