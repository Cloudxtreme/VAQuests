...
sub EVENT_SAY {
  if ($faction == 1) { #req ally CoV
    if($text=~/hail/i) {
      plugin::DiaWind("Greetings to you. I seek one who calls himself a wizard. What do you call yourself, manling?");
    }
    if ($text=~/i am a wizard/i) {
      plugin::DiaWind("Excellent. Are you sure enough of your skills to undertake my tasks? If not, get out of my sight!");
    }
    if ($text=~/undertake your tasks/i) {
      plugin::DiaWind("I thought so. One should never back down from a challenge. Once you have completed them I will have a cap, a robe, sleeves, wristbands, gloves, leggings and boots.");
    }
    if ($text=~/cap/i) {
