sub EVENT_SAY { 
	if(($text=~/hail/i) && ($faction < 6)) { 
		plugin::DiaWind("Just another boring day waiting for supplies.  I wonder when they will arrive.");
	} 
}

sub EVENT_ITEM {
	if(plugin::check_handin(\%itemcount, 25106 => 1)){ # Handin: Bekerak's New Spear 
        plugin::DiaWind("Well, well, I see Wenglawks finally got around to sending me my new spear. I hope it's not as shoddy as the last one. Your payment? Bahaha, did Wenglawks not tell you? Payment was to be a giant icewurm tooth carved into an amulet. Come now, little one. I will find an ice wurm for us to slay together. Then you can pull one of its teeth out for me and I will fashion the talisman for that greedy merchant.");
	...
	if(plugin::check_handin(\%itemcount, 25266 =>1)) { # Giant Sack of Supplies
		plugin::DiaWind("Supplies from Svekk?  Where are the rest of them little one!  There must be more, we are here pushing the Coldain back into their hole in the wall and Svekk sends a $race to deliver a small portion of supplies.  Take this note back to that fool.  Leave quickly before I take my rage out upon you.");
		quest::ding();
	...
	if(plugin::check_handin(\%itemcount, 25269 =>1)) { # Large Supply Sack for Bekerak
		plugin::DiaWind("I am in your debt, $race.  These supplies will help our effort to destroy the Coldain.  I wish I had more to give than what I do.'  Bekerak pulls a strange looking totem on a string from one of his pockets and hands it to you.  'May Lord Rallos smile upon you, $name.'");
		quest::ding();
		quest::faction(189,10); # Kromzek
		quest::faction(188,10); # Kromrif
		quest::faction(49,-30); # Coldain
