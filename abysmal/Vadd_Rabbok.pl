...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::DiaWind("I seek a powerful warrior. One who does not shy from battle or duty. Are you such a [warrior]?");
  }
  if ($text=~/warrior/i) {
    plugin::DiaWind("The world does not turn without the service of those such as yourself. If you are in need of new equipment such as [helm], [legplates], [chestplate], [bracer], [vambraces], [gloves], or pair of [boots] let me know.");
  }
  if ($text=~/bracer/i) {
  ...
  if ($text=~/boots/i) {
    plugin::DiaWind("Bring me back Muramite Boot Armor and I believe I can rework them into something quite fine for you. You will need a sheet of infused Muramite metal along with new boots from me in a forge to get a good fit. If you need help in the infusing process, just talk to Nalasrine."); 
  }
  if ($text=~/vambraces/i) {
    plugin::DiaWind("I shall make you some glorious vambraces. Hmm, find some Muramite Sleeve Armor. Bring that to me and I will start shaping it to fit you. Once you have some Muramite metal talk to Nalasrine about infusing it. Then simply take the new vambraces from me and forge them with some of the infused metal.");
  }
}
