    ...
    if(defined($qglobals{pop_cod_final}) && ($qglobals{pop_cod_final} == 1)) { #Must have killed Bertox and rescued Mylik and Adler
      ...
      if(($text=~/skills with a fletching knife/i) && plugin::check_hasitem($client, 32800)) { #Marked Signet
        if($client->GetRawSkill(64) >= 220) { #Fletching
          plugin::DiaWind("Aye I can see you are skilled with the fletching knife $name. It is good too, I need to replace Councilman Taldarius's bow from Plane of Air. Combine a planing tool, two wind metal bow cams, an air arachnid silk string, a featherwood staff and the signet. I hope the bow will be up to his standards, he sure loved his old bow.");
        }
        else {
          plugin::DiaWind("You do not seem to be skilled enough.");
        }
      ...
      plugin::DiaWind("You do not seem experienced enough in the Planes.");
    }
  }
}
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 15985 => 1)) { #Imbued Breastplate
    plugin::DiaWind("What a wonderful job! Councilman Taldarius shall wear this on our next adventure, I am sure he will find it more protective than his old one. Take this signet as a token of my gratitude. If you are not too busy I have another task, do you have any brewing skill?");
    ...
    plugin::DiaWind("Outstanding work $name! I can feel the purity of the water radiating through the clay. Are ye skilled with the needle as well as an accomplished potter?");
    quest::summonitem(16252); #Metal Signet
  }
  elsif(plugin::check_handin(\%itemcount, 15986 => 1)) { #Fire Undergarment Tunic
    plugin::DiaWind("Good work $name. This will definitely serve Councilman Taldarius well. Here take this! If you have skills with a fletching knife I may have a job for you to do.");
    quest::summonitem(32800); #Marked Signet
  }
  elsif(plugin::check_handin(\%itemcount, 16247 => 1)) { #Signet Featherwood Bow
    plugin::DiaWind("Masterful work $name! I can see your skill in the curves of the bow. Take this as a sign of my respect for your skill. The last item I need before we can be off is some food. I warn you this will truly test your skills. Do you fancy yourself a master chef?");
    quest::summonitem(16254); #Runed Signet
  }
  elsif(plugin::check_handin(\%itemcount, 16248 => 1)) { #Food Satchel
    plugin::DiaWind("Truly amazing! Now the Councilman and I can be off on our expedition to the Elemental Planes!' He takes out a tool and marks his signet before handing it to you, 'Before we depart you may want to ask the Councilman about the signet.");
