...
sub EVENT_SAY
{
	...
	if ($faction <= 2) {
		...
		if ($text=~/killed him/i) {
			plugin::DiaWind("As I said, they are only rumors. I refuse to believe that he is dead until I have proof. However, if he is dead I shudder to think of his knowledge and tools in the possession of the Muramites. Perhaps you could venture there and investigate these rumors for me. Ladislev always wore a ring that proclaimed his status as a master architect. If you find that he is dead bring me that ring along with his [tools]. But if he is alive, do all that you can to free him and return with him.");
		}
		
		if ($text=~/tools/i) {
			plugin::DiaWind("Ladislev kept the tools of his trade in an ornate toolbox. It was a gift to him from the council many years ago to show their appreciation for some of his work on the palace. He also kept with him a detailed book of ancient dragorn architecture. He drew much of his inspiration from this book and it would be a great loss to our culture if it were destroyed.");
		}
		
		if ($text=~/family/i) {
			plugin::DiaWind("My family used to have a wonderful home in Dranik. My father had planned on passing it on to me and I to my children. This was all before The Darkening and the coming of Mata Muram. With the coming of the Muramites our house was ransacked and we were chased from the city by a pack of ukun. We were forced to leave at a moment's notice and only just avoided being eaten alive by those vile beasts. One of the items we had to leave was the great [crest] of my family.");
		}
		
		if ($text=~/crest/i) {
			plugin::DiaWind("It was a gift of the council many years ago, given to my family for service to the dragorn people. As such, it holds a great deal of sentimental value for me. If you could return the crest to me and three of the chain leashes used on the ukun as a measure of vengeance against them I would be in your debt.");
		}
	}
}
...
sub EVENT_ITEM
{
	if ($faction <= 2) {
		...
		if(plugin::check_handin(\%itemcount, 36541 => 1, 36540 => 1, 36539 => 1, 36538 => 1)) {#Sliver of Wood, Cornerstone Fragment, Tower Mortar, Harbingers Spire Blueprints
			plugin::DiaWind("Excellent! You've found all that I asked for. I will review these plans and materials immediately and see what can be done. Take this belt as a reward. It has been in my family for ages, but seems a fitting reward for your efforts.");
		...
		#Recover the Family Crest
			...
			plugin::DiaWind("Take this amult as a reward.");#Text made up
			quest::faction(410, 10);#Dranik Loyalists
			quest::faction(432, -20);#Overlord Mata Muram
			quest::exp(10000);
			#Reward depends on class
