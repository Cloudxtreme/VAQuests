...
sub EVENT_SAY {
 if($race=~/froglok/i) {
  if($text=~/hail/i) {
   plugin::DiaWind("Nice to meet you, $name! Please have a look at my wares, I have taken great pains to brings only the best that the mountains have to offer. Or perhaps you are here to make an [exchange]?");
  }
  
  if($text=~/exchange/i) {
   plugin::DiaWind("I am always in need of more skins, so I will swap three identical unruined pelts for one of my higher quality versions.");
  }
 }
}
...
sub EVENT_ITEM {
 if($race eq "Froglok") {
  if(plugin::check_handin(\%itemcount, 12084 => 3)) {
   plugin::DiaWind("Hmm. . . That seems like a fair trade. Let me see what I've got in my satchel. No. Wait, yes! I do have a fine specimen here that I think you'll agree is of superior quality.");
   quest::exp(150);
