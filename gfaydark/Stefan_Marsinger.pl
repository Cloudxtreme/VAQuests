sub EVENT_SAY {
 if($text =~ /Hail/i) {
   plugin::DiaWind("Salutations, my name is Stefan Marsinger. My sister Seana and I are here on a mission for the League of Antonican Bards. Are you [interested] in assisting us in our task?");
 }
...
sub EVENT_ITEM {
  ...
  if (plugin::check_handin(\%itemcount, 54036 => 1, 54045 => 1, 54046 => 1)) { #caster 2
    ...
    plugin::DiaWind("Very interesting, I must get to this at once. I am sure the bones will prove most valuable in learning of the drakes as well. Take this with you once more; the protective properties are even more powerful now. Are you up to a [final task]?");
    quest::exp(60000);
  }
