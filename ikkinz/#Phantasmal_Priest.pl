...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    ...
    plugin::DiaWind("Are you here to help [cleanse] the temple?");
  }
  if ($text=~/cleanse/i) {
    quest::emote("pushes something dry and brittle into your hands");
    plugin::DiaWind("take these and see if you can piece them together.  If so give them back and maybe I can help you.");
 ...
 if ($text=~/north/i) {
  if ($qglobals{ikkymove} == 1) {
     ...
     quest::MovePCInstance(294,$InInstanceIkky7,195,-86,21,0);
       ...
       plugin::DiaWind("Fool, I can not help you if you do not help me.");
       }
 } 
 if ($text=~/south/i) {
  if ($qglobals{ikkymove} == 1) {
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 60242 =>1 )) {
    plugin::DiaWind("Ah this is what I need.");
    quest::emote("takes the piece of parchment and whispers softly.  The piece flares with magic and he turns to look at you.");
    plugin::DiaWind("What you need is both [north] and [south], speak return and it shall be so.");
