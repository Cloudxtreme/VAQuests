sub EVENT_SAY {
  if ($text=~/hail/i) {
    if (quest::istaskactivityactive(171,4)) { #Alchemy Collect Step 5
      plugin::DiaWind("You've been a great help already. I sense that you have greater tasks to attend to and there are fledglings about that can continue your work. As a reward for you assistance, please allow me to teach you the secret of filtering the Taelosian sludge.");
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 58076 => 1)) { #Sample of Taelosian Sludge 
    plugin::DiaWind("Ah, yes. Here is your filtered sludge.");
