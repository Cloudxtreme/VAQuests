...
sub EVENT_SAY {
	...
	if($qglobals{$instid.'_inktuta_status'} == 0) {
		if ($text=~/hail/i) {
			quest::emote("screams");
			plugin::DiaWind("No, don't kill me!' He relaxes slightly and gives you a strange look. 'Have you been afflicted by the curse? Are you mad!? Have you any idea [" . quest::saylink("who Kelekdrix") . "] is? She will be here any moment to destroy you all. Leave now, or there will be dire [" . quest::saylink("consequences") . "]!");
		} elsif ($text=~/consequences/i || $text=~/who Kelekdrix/i) {
			plugin::DiaWind("Very well, $name. You brought this upon yourself.");
