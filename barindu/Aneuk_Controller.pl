sub EVENT_ATTACK {
  ...
  plugin::DiaWind("Finally, a chance to practice our tactics. Constructs, destroy their support troops!");
}

sub EVENT_SIGNAL {
  if($signal == 1) { # signal from #ikaav_ixvet_pox
    plugin::DiaWind("Bring that tray here, girl! I'm famished.");
    plugin::DiaWind("Don't keep that all to yourself. Bring some of that fruit over here too!");
