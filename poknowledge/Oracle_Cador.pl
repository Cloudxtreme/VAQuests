...
sub EVENT_SAY {
  ...
  if($text=~/black lava powder/i) {
    plugin::DiaWind("You are most astute. I do have a stock of black lava powder with me, but I will only share it with those who prove themselves worthy. Onirelin Gali is currently in possession of an artifact I need to better commune with the spirits. Please recover it for me.");
  }
}
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 28089 => 1)) {#Cador's Artifact
    plugin::DiaWind("The spirits are restful now that this piece of legend is in safe hands. You may take this to Mirao for whatever purpose he desires it for. May your vision always be clear!");
