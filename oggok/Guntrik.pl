...
sub EVENT_SAY { 
	if($text=~/hail/i){
		plugin::DiaWind("Mmmph!!  Who you?  Oh. you $name.  You supposed to be promising Craknek.  Me need you.  There bad things in swamp.  You want [help Crakneks] or you [want Guntrik bash your face]!!?");
	}
	if($text=~/what help crakneks/i){
		plugin::DiaWind("Crakneks warriors!!  They be far back.  Through last caves.  They smash and bash.  They no like Greenbloods.  Bouncers keep thems from bashing.");
	}
	if($text=~/want gruntrik to bash my face/i){
		plugin::DiaWind("OK!! Me bash!!  You hurt!!");
	}
	if ($text=~/help crakneks/i) {
		plugin::DiaWind("Go to swamps. Find noble hunters of humans and elves. All have house crests. Return to me and I give things. If you find special item says where hunters sleep, give to me. If meat founded then give to Clurg's cook. She make fine stew and give coins for meat.");
	}
  if ($text=~/trades/i) {
...
sub EVENT_ITEM { 
	if (plugin::check_handin(\%itemcount, 18787 => 1)) {
		...
		plugin::DiaWind("One less trouble. Hunhuh!! You do good work. Keep up. Remember to bring any special things to Guntrik. Here junk for good work. Go away now.");
