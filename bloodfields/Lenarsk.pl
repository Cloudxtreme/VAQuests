...
sub EVENT_ITEM {
 ...
 my $ifeet = $rfeet{$class};
	...
	if (plugin::check_handin(\%itemcount, 51481 => 1, 51502 => 2))
		{
		plugin::DiaWind("Makyah's Axe! Now that this is returned to us, I hold hope that it may be wielded in our stuggle, to bolster our people's hope once more. Thankyou $name."); #made up
		quest::exp(100000);
	...
	if (plugin::check_handin(\%itemcount, 51475 => 1, 51496 => 2))
		{
		plugin::DiaWind("Excellent effort $race. This was exactly the walking stick i sought. Here take these as offered."); #made up
		quest::exp(100000);
	...
	if (plugin::check_handin(\%itemcount, 51478 => 1, 51499 => 2))
		{
		plugin::DiaWind("I hope this brings poor Vrago some measure of peace. Take these and use them to aid in you in the destruction of the Muramite armies. I am sure this too would bring Vargo peace."); #made up
		quest::exp(100000);
