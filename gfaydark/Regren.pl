...
sub EVENT_SAY {
	if($text=~/hail/i){
		plugin::DiaWind("Welcome, warrior! Show the Emerald Warriors your mettle and bring me a ruined wolf pelt, some bat fur, some bone chips, and a spiderling eye from the depths of Greater Faydark. If you succeed, my admiration and a reward will be yours. To battle!");
	}
	if($text=~/trades/i) {
...
sub EVENT_ITEM {
	...
	if (plugin::check_handin(\%itemcount,13073=>1,13782=>1,13253=>1,13069=>1)) {
		plugin::DiaWind("Fine work! You are on your way to becoming an adequate combatant.");
