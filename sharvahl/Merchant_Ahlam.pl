...
plugin::DiaWind("Hello $name. would you care for a drink?  Or perhaps you're the one they sent to help
 me?  If you've come to help please show me your acrylia slate.");
}


if($text=~/interested/i){
   plugin::DiaWind("Is that so? I suppose I would be willing to teach you a few things about brewing,
but you'll have to show me one of my seals before I begin your instruction.");
}

if($text=~/purify water/i){
   plugin::DiaWind("Take a bottle to Gamali in Hollowshade Moor. You'll find her somewhere around
...
if($text=~/gravel leaf tea/i){
   plugin::DiaWind("To make the tea you'll need a flask of crystal clear water and some gravel pit
...
if($text=~/hollow palm tea/i){
   plugin::DiaWind("You'll need some Crystal water, just as you do with all of our teas, and some
...
if(plugin::check_handin(\%itemcount,2877=>1)){
   ...
   plugin::DiaWind("Ah good I was hoping you'd be along soon. I need you to collect some Gravel
...
if(plugin::check_handin(\%itemcount,31764=>1)){
   plugin::DiaWind("Many thanks $name, I'll be going to brew this tea up for the guards now. Take
...
if(plugin::check_handin(\%itemcount,31774=>1)){
   plugin::DiaWind("Well done, it is now time to start down the path of learning the art of Vah Shir
...
if(plugin::check_handin(\%itemcount,31762=>4)){
   plugin::DiaWind("Very good $name, this water is perfect for making Gravel leaf tea. I've just
...
if(plugin::check_handin(\%itemcount,31775=>1)){
   plugin::DiaWind("Very good $name, the guards had been waiting for that tea all day. I think you're
...
if(plugin::check_handin(\%itemcount,31769=>1, 31768=>1)){
   ...
   plugin::DiaWind("Mmmmm, I do so love Hollow palm tea. This is a bit mild for Hollow palm tea
...
if(plugin::check_handin(\%itemcount,31772=>1)){
   plugin::DiaWind("Great job, here is my seal of approval.  Hand that back in along with your stainer
for an upgrade.");
...
if(plugin::check_handin(\%itemcount,31776=>1, 31771=>1)){
   plugin::DiaWind("Here is the Acrylia Handled Strainer.  Use it with regular water to replace the
crystal clear water.");
