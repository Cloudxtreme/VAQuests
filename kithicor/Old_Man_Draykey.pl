		...
		if(!defined $qglobals{halloween_draykey}){
			...
			if ($text=~/instructions/i) {
				...
				if(!defined $qglobals{halloween_draykey_rations}){
              	...
              	plugin::DiaWind("I'm sorry, but you can only complete my event once!"); 
		}
	}
	elsif(quest::istaskactive(220)){
		plugin::DiaWind("Once you've collected all ten return to me with the full bag.");
	}
}
...
sub EVENT_ITEM {
	if(quest::istaskactivityactive(220,11)){
		if(plugin::check_handin(\%itemcount, 84096 => 1)) {
			plugin::DiaWind("Ah ha! You've found them all! Excellent work, here's a bit of candy for you. Enjoy!");
