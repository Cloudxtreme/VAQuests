...
sub EVENT_ITEM {
 ...
 my $ifeet = $rfeet{$class};
	...
	if (plugin::check_handin(\%itemcount, 51446 => 1, 51467 => 2))
		{
		plugin::DiaWind("Fantastic work $race . The loss of the shard should prevent the opportunity we were waiting for. Please take these and wear them with pride."); #made up
		quest::exp(100000);
		quest::summonitem($ihands);
		}
	if (plugin::check_handin(\%itemcount, 51445 => 1, 51466 => 2))
		{
		plugin::DiaWind("The Blood Standard of Dranik! I did not believe it still existed! This will bring hope to the few dragorn who still resist Mata Muram. I only wish I was ready to join them. Thank you. As promised, here is a leather wristguard as a reward.");
		quest::exp(100000);
	...
	if (plugin::check_handin(\%itemcount, 51441 => 1, 51462 => 3))
		{
		plugin::DiaWind("He lives yet! You see it glows still, if only faintly.' Barowsar pauses for a moment in quite contemplation before continuing. 'Here $name, this was my finest work, and you deserve no loss for the favor you have done me this day."); #made up
		quest::exp(100000);
	...
	if (plugin::check_handin(\%itemcount, 51443 => 1, 51464 => 2))
		{
		plugin::DiaWind("Well done $name. I understand the Muramites guard these well. I'll bury this stone so the dragorn of Kuua can move on from the horrors of the past and instead look to the future. As promised, here is a pair of finely crafted leather boots as your reward.");
		quest::exp(100000);
