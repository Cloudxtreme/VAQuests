...
sub EVENT_ITEM {
  ...
  if(($ulevel > 45) && plugin::check_handin(\%itemcount, 60210 => 1)) { #Medal of Mastery    
    if(defined($qglobals{raving}) && ($qglobals{raving} == 1)) {
      plugin::DiaWind("You must wait to attempt this again.");
  ...
  if(($medal == 1) && ($cash >= 1000000)) {
    plugin::DiaWind("Good timing. Me can hear him giggling. Go down to da stream's end where it opens into a big river between da cliffs. Crazy goblin is usually in dat area stirring up troubles. Go tell him dat you are a berserker and be ready. He go nuts. Good lucks. Me go find help.");
