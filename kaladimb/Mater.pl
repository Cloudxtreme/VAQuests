...
sub EVENT_SAY {
   if ($text=~/hail/i) {
      if ($faction <= 7) {
         plugin::DiaWind("Welcome to the mines of Kaladim!");
      }
      else {
      ...
      if ($faction <= 4) { # Requires high amiable on live
         plugin::DiaWind("So you have heard of Mining Pick 628 and feel you are ready to wield one? You shall earn one with the return of the ogre head of [Boog Mudtoe] and the 300 gold pieces he still owes me. Don't return unless you have the head and the 300 gold!!");
      }
      elsif ($faction <= 5) {
      ...
      if ($faction <= 4) { # Requires high amiable on live
         plugin::DiaWind("Boog Mudtoe is one of the last remaining ogres of the Mudtoe clan. We helped him escape the assault of the Stormguard for a small fee. Unfortunately for him, we have come to believe his head to be more valuable than the fee. We hear there is someone near the port of Butcherblock who knows of the Mudtoe's whereabouts.");
      }
      elsif ($faction <= 5) {
         plugin::DiaWind("Don't take this personally, but I can't quite trust you with such matters. Maybe a few less Butcherblock bandits would prove your worth.");
      }
...
sub EVENT_ITEM {
   ...
   if (($faction <= 4) && (plugin::check_handin(\%itemcount, 13316 => 1))) { # Ogre Head
      if ($cash >= 30000) { # 300 Gold
         plugin::DiaWind("Very good!! You found him. His head shall bring us a great reward from the Stormguard. And as for you, here is your Mining Pick 628. Only a member of 628 can wield this fine weapon. We are the only ones who can wield it in such a way as to pierce our foes.");
...
sub EVENT_TIMER {
   plugin::DiaWind("Blast all these pesky rats!! Jeet, you need to get one of the new rogues.. I mean miners, to get rid of them!!");
