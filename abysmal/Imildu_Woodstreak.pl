sub EVENT_SAY {
  if ($text=~/hail/i) {
    if (quest::istaskactivityactive(159,4)) { #Fletching Freebie Step 5
      ...
      plugin::DiaWind("May Tunare bless you, friend. I can no longer keep you here. Certainly you too wish to get back out under the sun and see the new land. Your help has allowed me much freedon and for that I thank you. Look at this compound. using it when you work with the Chunks of Taelosian Rock and the Chunks of Discordian Rock will allow you to work them into the shapes you desire.");
      ...
      plugin::DiaWind("Welcome $name, Tunare smiles upon us all. I don't want to inpose, I know that we are all very busy, but if you have the time and inclination, I could use your help finishing some [tasks] that have been overwhelming me.");
    }
  }
  if ($text=~/tasks/i) {
  ...
  if ($text=~/components/i) {
    ...
    if (quest::istaskactivityactive(159,1)) { #Fletching Freebie Step 2
      plugin::DiaWind("Hail, Traveler. Thank you for taking the time to help me. I have several bow staves and stings that need to be assembled. A simple enough task, but time consuming. All you need to do is place the Wayfarer Bow Staff and the Wayfarer Bow String into your fletching kit and combine them. These bows will be needed soon, so please bring them directly to me when you have them assembled.");
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 58030 => 1)) { #Chunk of Taelosian Rock
    plugin::DiaWind("Ah, yes. Here is your stone.");
