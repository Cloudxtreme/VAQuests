sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::DiaWind("You have the aura of a beastlord I should trust. If Elder Muada hasss sent you, you must prove it to me.");
  }
}

sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 52906 =>1 )) {
    plugin::DiaWind("I have seen the creature nearby, but I have not been able to defeat it.  I am not strong enough.  Perhaps you may have better luck.  You must hurry.  It has used the warder spirit to create twisted and dark companions of its own. If you should overcome this creature, return to me.");
  ...
  if (plugin::check_handin(\%itemcount, 52907 =>1 )) {
    plugin::DiaWind("You have certainly accomplished what I was not able to. I must go now, but be sure to take this letter and the essence back to Rashara.");
