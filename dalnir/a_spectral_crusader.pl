...
sub EVENT_SAY {
 if($text=~/greenmist/i){ #Test of the Hero (Greenmist Quest 6/8)
  plugin::DiaWind("Mighty was the mist which covered the land. Mighty is the blade wielded by a crusader.");
  }
  ...
  plugin::DiaWind("Shattered!! Three within the crypt. A master must go. To the [grand forge] they must go. Three become one with the spirit hammer.");
  }
