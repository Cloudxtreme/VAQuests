...
sub EVENT_ITEM{
  if(plugin::check_handin(\%itemcount, 59022 => 1)) {
    ...
    plugin::DiaWind("I can't read all of it, Take the rest to Jusean Evanesque of South Qeynos, he will understand.");
    quest::summonitem(59023);
 }
  if(plugin::check_handin(\%itemcount, 59024 => 1)) {
    plugin::DiaWind("You have done well $name, my friend will now be saved, let me share my song with you.");
    quest::exp(1000);
