sub EVENT_SAY {
  ...
  if(defined $qglobals{bic}) {
    ...
    # ^-------------- that is it for bic == 1 stuff
      ...
      plugin::DiaWind("Where are my manners? I should have known someone as adventurous as you would be interested in this. One of our researching experts by the name of Beanalle has recently discovered some very intriguing information about the temples beyond Yxtta and Kod'Taz.");
	}
  } #end bic defined
  #seperate check for the two keywords since they use >=1
  if(defined $qglobals{bic} && $qglobals{bic} >= 1) {
    if($text=~/sealer/i) {
      plugin::DiaWind("Be careful with this one.");
      quest::summonitem(67659);
    } elsif($text=~/stone/i) {
      plugin::DiaWind("Be careful with this one.");
      quest::summonitem(67512);
