...
sub EVENT_SAY { 
	...
	if ($text=~/task/i){
		plugin::DiaWind("As a sign of loyalty, and to avenge the innocent blood that was spilt by the Luggalds, I would like you to cut down some of the fiends and bring proof of their demise. Use whatever means necessary. Grant them no quarter and expect none in return. For each of their dark hearts, I will trade pieces of the ice that you seem so interested in. I will accept nothing else.");
	}
}
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 58007 => 1)) {#Luggald Heart
    plugin::DiaWind("Yes!  For this I will gladly give you a fragment of the dark ice!");#text made up
