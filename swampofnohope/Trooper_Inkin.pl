sub EVENT_SAY {
  if ($text=~/hail/i) {
    plugin::DiaWind("Many frogloks in here. Beware! They can be a sneaky bunch. I hear tales of a froglok village nearby.");
  }
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 18243 => 1, 12425 => 1)) { #Legion Order (3 signed), Skipping Stone
    plugin::DiaWind("So smooth. I shall be the skipping stone champion! Here. You may have my autograph.");
    quest::summonitem(18244); #Legion Order (4 signed)
    quest::exp(500);
    quest::givecash(7,0,0,0);
  }
  elsif (plugin::check_handin(\%itemcount, 18243 => 1)) {
    plugin::DiaWind("No deal! You bring me the legion order and the froglok skipping stone.");
