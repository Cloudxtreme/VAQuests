...
sub EVENT_SAY {
  ...
  if($text=~/dark arts/i) {
    if($class eq "Necromancer" && $ulevel >= 50) {
      ...
      plugin::DiaWind("Ha! You call yourself a Necromancer? Come back when you have tortured a few more souls!");
    }
  }
  if($text=~/prove/i) {
    plugin::DiaWind("The wandering knight, Sir Edwin Motte, recently slew one of my lesser servants. This foolish knave travels from town to town pretending to be good. Separate his head from his body and return it to me so I may work my magics upon it. This fool will become another plaything for me, even in death.");
  }
  if($text=~/advance/i) {
    plugin::DiaWind("I share great secrets with my more powerful servants who show me they are willing to serve without question. Tell my servant the name of any symbol he gives you and he will instruct you. If I give you a symbol, speak its name to me so I may instruct you.");
  }
  if($text=~/symbol of apprentice/i) {
    plugin::DiaWind("You have a long way to go before I teach you the true secrets of life and death. Visit Venenzi Oberendi in the Nektulos Forest and show her the symbol. She is collecting reagents for one of my experiments and is taking far too long. I do not wish her dead, yet, I simply need the reagents soon. Retrieve the reagents and give them to my assistant Emkel Kabae. I do not wish to handle them myself.");
  }
  if($text=~/reagents/i) {
    plugin::DiaWind("'The reagents are the same types the great sorcerer Miragul used to cheat death and become a lich. I do not wish to become a lich as Miragul did, I have greater aspirations than to simply live forever as a hollow shell. I will rise up a being of pirit only, with a will off my own.' Kazen's eyes glow sickly red as he stared off toward the horizon.");
  }
  if($text=~/symbol of testing/i) {
    plugin::DiaWind("Show me that you can overcome larger obstacles and manipulate servants. Gather a force for of mercenaries if you like, or so-called friends, if you desire, and prepare for battle, my promising young student. Return to me with the symbol of testing when you are prepared for battle with my creations. Have no fear, I will not slay you myself. If you do live, tell me the name of the symbol you receive.");
  }
  if($text=~/symbol of insanity/i) {
    plugin::DiaWind("It is time for you to meet my partner, so to speak. Drendico Metalbones is his name and I assure you he is quite mad. His mind has been rended by fright, dread, and terror in his trips to the planes and his heart is full of hatred now. He has a tome which I wish you to use. Show him your symbol slowly and listen to his rantings well, they hide great knowledge.");
  }
}
...
sub EVENT_ITEM {
  if(plugin::check_handin(\%itemcount, 20641 => 1)) { #Head of Sir Edwin Motte
    ...
    plugin::DiaWind("There appears to be a small rune etched in the wood.");
    quest::emote(" takes out a piece of parchment and draws a picture of the rune.");
    plugin::DiaWind("I am not familiar with this rune. Take this drawing of the rune to Ruandia in the Plane of Knowledge. She is an expert on runes and should know what it means. From here on out I will want you to handle the research of this staff through my assistant Tebarin in the Plane of Knowledge. I am busy and do not wish to be disturbed further. Tebarin will communicate to me any progress you have made with helping to recover the staff.'");
