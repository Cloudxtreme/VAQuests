...
quest::summonitem(17277);
	plugin::DiaWind("Seal those things up in the container that I just gave you and bring it straight back to me. I'll wait here for you and see if I can decipher anymore of this schematic."); #not sure on how to check to see if the container combine works
}
if (plugin::check_handin(\%itemcount, 28283 => 1)) {
plugin::DiaWind("Oh, you've returned! Very good, $name! Saria and I have made some real progress on the document that you brought us. Watch this and you'll see what I mean!");
...
quest::summonitem(28284);
	plugin::DiaWind("That should work for you, but from what we gathered, there's another layer of mechanics that works to shield and envelope the existing talisman. It's going to require more parts. So take this container and let me know when you're [ready] to write down the names of the next set of parts.");
quest::exp(100000);
}
if (plugin::check_handin(\%itemcount, 28289 => 1)) {
  ...
  plugin::DiaWind("take this schematic to Elder Clinka in the Plane of Tranquility. He should be able to decifer it for you.");
