...
sub EVENT_SAY {
  if (($text=~/hail/i) && ($event == 1)) {
    plugin::DiaWind("You have found my three stone shaping tools.");
  ...
  if (($text=~/hail/i) && ($event == 0)) {
    plugin::DiaWind("Help me escape! Wait, who sent you?");
  }
}
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 68298 => 1)) {
    plugin::DiaWind("I will help you if you can help me. I've lost my tools. Some of the denziens of the sewers stole them after I got trapped under this rock. Get my tools so I can escape and I'll show you to the pool."); #not live text
