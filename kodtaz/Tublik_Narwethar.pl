sub EVENT_SAY {
  ...
  if ($text=~/hail/i) {
    if (defined($qglobals{ikky}) && ($qglobals{ikky} == 7)) { 
      plugin::DiaWind("Welcome back, $name. Have you found any clues from the [" . quest::saylink("Crumbled Sanctuary") . "] yet? Please find anything you can and return to me as soon as possible!");
    }
    if (defined($qglobals{ikky}) && ($qglobals{ikky} == 10)) { 
      plugin::DiaWind("I wasn't expecting you back so soon, $name, but I'm glad you're here. I've been analyzing the glyphs on the last three artifacts you recovered and, the way I read them, it seems the trusik priests used the [" . quest::saylink("four temples") . "] around the Altar of Destruction to gain access to a temple to face some kind of final rite passage.");
    }
    if (defined($qglobals{ikkypit}) && ($qglobals{ikkypit} == 1)) { 
      plugin::DiaWind("Welcome back, $name. I've been looking over those artifacts you returned to me in the sealed bag and I'm afraid I have some disturbing news. It seems the artifacts weren't artifacts at all, but objects of ghastly power held by the dark spirits you collected them from. What's worse is that since you've collocted these objects, the energy emanating from the pit has [" . quest::saylink("gotten stronger") . "]!");
    }
  ...
  if (($text=~/stone tablet/i) && defined($qglobals{ikky}) && ($qglobals{ikky} == 6)) {
    plugin::DiaWind("You need a stone tablet? What for? What have you got that's so special it requires the use of a stone tablet?");
  }
  if (($text=~/crumbled sanctuary/i) && defined($qglobals{ikky}) && ($qglobals{ikky} >= 7)) {
    plugin::DiaWind("Oh yeah, I know all about that thing. It was a temple that was decimated by the Muramites during their initial invasion. We're unsure what caused them to destroy that temple, but not the other three. I need you to go to the temple and [" . quest::saylink("look for clues") . "] as to what the reason may have been for destroying the temple.");
  }
  if (($text=~/look for clue/i) && defined($qglobals{ikky}) && ($qglobals{ikky} >= 7)) {
    plugin::DiaWind("I have no idea what the clues may look like, but I'm hoping they'll bring about some insight as to the reason behind the Muramites destroying the temple. Once you've recovered all the clues you can find, please come back to me so I can go over them. If we're lucky, I'll be able to decipher what the real cause was and we can relay that important news to the brotherhood. Off you go, $name, and good luck!");
  }
...
sub EVENT_ITEM {
  if (plugin::check_handin(\%itemcount, 60174 => 1)){
    if (plugin::check_hasitem($client, 60252)) {
      plugin::DiaWind("You have done great things for us, but your journey is not yet over.");  #need live text
      ...
      plugin::DiaWind("You have done well in the Temple, please return when you have recovered the fragment also.");
  ...
  if (defined($qglobals{ikky}) && ($qglobals{ikky} == 8)) {
    if (plugin::check_handin(\%itemcount, 60166 => 1)) {
      plugin::DiaWind("Nicely sewn, $name. I would have tried doing it myself, but I have the most unsteady fingers when it comes to things like that. In any case, let me have a look at what this says.' Tublik looks over the clue for a time before continuing. 'Unbelievable. Simply astounding. These notes weren't made by the Muramites at all. In fact, they were created by trusik priests from their own flesh in some kind of [" . quest::saylink("ritual") . "].");
  ...
  if (defined($qglobals{ikky}) && ($qglobals{ikky} < 14)) {
    if (plugin::check_handin(\%itemcount, 60173 => 1)) {
      plugin::DiaWind("You can't have this? What kind of trickery is this? I'm alerting the proper autorities you thief.");
    }
  }
  if (defined($qglobals{ikky}) && ($qglobals{ikky} >= 10)) {
    if (plugin::check_handin(\%itemcount, 60160 => 1)) {
      plugin::DiaWind("You're done already? I wasn't expecting such a prompt response, but nicely done %name! Give me a while to examine these artifacts. I should have some additional information for you once you return.");
      quest::setglobal("ikkypit",1,5,"F");
    }  
    if (plugin::check_handin(\%itemcount, 60170 => 1)) {
      if (!defined($qglobals{ikkyraid})) {
	    plugin::DiaWind("You've done it! You've recovered the first of the three artifacts! You must keep this with you, as you'll need it once you've recovered all three artifacts. You only need two more -- one from [" . quest::saylink("Sanctuary of the Glorified") . "], and one from the [" . quest::saylink("Sanctuary of the Transcendent") . "]. Which do you think you'd like to do next?");
    ...
    if (plugin::check_handin(\%itemcount, 60171 => 1)) {
      if (defined($qglobals{ikkyraid}) && ($qglobals{ikkyraid} == 1)) {
	    plugin::DiaWind("You've done it! You've recovered the second of the three artifacts! You must keep this with you, as you'll need it once you've recovered all three artifacts. You only need one more from [" . quest::saylink("Sanctuary of the Transcendent") . "]. Are you ready to progress forward with the final artifact?");
    ...
    if (plugin::check_handin(\%itemcount, 60172 => 1)) {
      if (defined($qglobals{ikkyraid}) && ($qglobals{ikkyraid} == 2)) {
	    plugin::DiaWind("You've done it! You've recovered all three of the artifacts! You must keep this with you, because you'll need it now. Somewhere deep down I knew you would be able to get them all. I've been studying the markings from the artifacts and I've come to the conclusion that they must be combined to form a key to enter the [" . quest::saylink("Altar of Destruction") . "].");
