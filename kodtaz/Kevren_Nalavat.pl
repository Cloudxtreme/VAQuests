sub EVENT_SAY {
  if ($text=~/hail/i) {
    if (defined($qglobals{ikky}) && ($qglobals{ikky} == 2)) {
      plugin::DiaWind("Welcome back, $name. Reports of your actions in the Temple of Singular Might suggest that you may be well on your way to becoming an outstanding addition to the coalition that investigates these temples. I continue standing my post and , boring as it is, it is of no interest to you I'm sure. If you want, we can talk [more] when we have some idel time. For now, if you're [ready to continue testing], we can proceed! What'll it be, $name?"); 
    }
    if (defined($qglobals{ikky}) && ($qglobals{ikky} == 3)) {
      plugin::DiaWind("Welcome back, $name. Reports of your actions in the Temple of Twin Struggles suggest that you may be well on your way to becoming an outstanding addition to the coalition that investigates these temples. I continue standing my post and, boring as it is, it is of no interest to you I'm sure. If you want, we can talk [more] when we have some idle time. For now, if you're [ready to continue testing], we can proceed! What'll it be, $name?"); 
    }	
    if (defined($qglobals{ikky}) && ($qglobals{ikky} == 4)) {
      plugin::DiaWind("Welcome back, $name. I must congratulate you on your recent completion of all three trials. I must admit that I was unsure of your ability to do as well as you did. Now that you have finished, you have the choice of taking on some [" . quest::saylink("other tasks") . "]. If you'd like, we have a chance now to talk [" . quest::saylink("more") . "]. You're also more than welcome to attempt the trials again. If so, just let me know you're [" . quest::saylink("ready to test") . "] again and we'll proceed down that path. What'll it be, $name?");
    }
    if (defined($qglobals{ikky}) && ($qglobals{ikky} == 5)) {
      plugin::DiaWind("Hmm, we are still missing some valuable information. I need you to retrieve the missing piece.");
    }
    if (defined($qglobals{ikky}) && ($qglobals{ikky} == 7)) {
      plugin::DiaWind("We have come upon another problem. The summoners that you encountered in the Temple of the Damned are working at another location closer to the main temple to summon more vile creatures. Do you think that you can [" . quest::saylink("stop the ritual") . "]?"); #need live text
    }
    if (defined($qglobals{ikky_flesh}) && ($qglobals{ikky_flesh} == 2)) {
      plugin::DiaWind("The problem is that this information reveals a most disturbing plot by the Muramites to summon a creature into this realm. They appear to be experts in teleportation. The parchment you returned to me described the use of Temple of the Damned to gather instruments of power that will allow the beast to coalesce in our world. It doesn't go into what those instruments are or when the summoning will take  place, so you must [" . quest::saylink("discover their plans") . "].");
  ...
  if ($text=~/specific order/i) {
    plugin::DiaWind("There exist three temples nearby that you must enter. I have defined these three as the Temple of [" . quest::saylink("Singular Might") . "], Temple of [" . quest::saylink("Twin Struggles") . "], and Temple of the [" . quest::saylink("Tri-Fates") . "]. In each lay the forces of the Legion of Mata Muram who must be destroyed. You must not be fooled, we do not control these temples. The areas are controlled by the [" . quest::saylink("Muramites") . "], but we believe that these places hold the most predictable of the invaders and can be used easily as our testing grounds.");
  }
  if ($text=~/singular might/i) {
    if (!defined($qglobals{ikky})) {
      plugin::DiaWind("Beyond the dark fog to the south lies the Temple of Singular Might. You can find it between the two other temples. In front of the temple you will find a single, smaller temple where another of the brotherhood is waiting for you. Seek out Gazak Klelkek and speak to him about the troubles within the temple.");
  ...
  if ($text=~/twin struggles/i) {
    if (defined($qglobals{ikky}) && ($qglobals{ikky} == 2)) {
      plugin::DiaWind("Beyond the dark fog to the south lies the Temple of Twin Struggles. It is past the first two temples. Seek out Maroley Nazuey and speak to her about the troubles within the temple.");  #need live text
    }
    else {
      plugin::DiaWind("I'm sorry $name, but you're not ready to face the second trial. You must first find Gazak Klelkek near the Temple of [Singular Might] and finish the first trial before you may proceed. Return to me when you have accomplished that feat.");
    }
  }
  if ($text=~/tri-fates/i) {
    if (defined($qglobals{ikky}) && ($qglobals{ikky} == 3)) {
      plugin::DiaWind("Beyond the dark fog to the south lies the Temple of Tri-Fates. It is the first temple you will come to. Seek out Kenra Kalekkio and speak to her about the troubles within the temple");  #need live text
    }
    else {
      plugin::DiaWind("I'm sorry $name, but you're not ready to learn about the third trial. You must first find Gazak Klelkek near the Temple of [Singular Might] and finish the first trial before you may proceed. Return to me when you have accomplished that feat.");
    }
  }
  if (($text=~/other tasks/i) && defined($qglobals{ikky}) && ($qglobals{ikky} >= 4)) {
    plugin::DiaWind("As I suspected, you're ready for some challenging work! I must warn though, what lies ahead is not for the faint of heart. The other tasks that are available deal with three other areas. Those areas are the [" . quest::saylink("Martyrs Passage") . "], the [" . quest::saylink("Pit of the Lost") . "], and the [" . quest::saylink("Crumbled Sanctuary") . "] of the Divine. Choose the one you wish to attempt and we can continue.");
  }
  if (($text=~/martyrs passage/i) && defined($qglobals{ikky}) && ($qglobals{ikky} == 4)) {
    plugin::DiaWind("Good, we're in need of a sturdy adventurer to help us with a dire situation along with the Martyrs Passage. First, this area was named so because it houses the spirits of martyred heroes lost long ago. The trusik lined the passageway with [" . quest::saylink("ancient relics") . "] to sooth the spirits while in the afterlife. Unfortunately, the Muramites are disturbing the remains and are looking to collect the spiritual remnants for some sinister use.");
  ...
  if (($text=~/ancient relics/i) && defined($qglobals{ikky}) && ($qglobals{ikky} >= 4)) {
    plugin::DiaWind("We're unsure what the invaders are gathering these particular relics for. From what I can surmise, none of them have any kind of magical properties. In any case, it's up to you and a group of adventurers to venture to the Martyrs Passage and [" . quest::saylink("investigate") . "] the situation. I've heard reports of ghosts in that area, so the invaders may have stirred up something unexpected.");
  }
  if (($text=~/investigate/i) && defined($qglobals{ikky}) && ($qglobals{ikky} >= 4)) {
    plugin::DiaWind("You'll need to head to the Martyrs Passage and recover four different relics. I advise taking a sturdy group with you because there may be invaders there in the process of burgling the passage. If you uncover any information on what the Mata Muram are planning to use these relics for, return that to me as well, Do you [" . quest::saylink("understand") . "] your objective?");
  }
  if (($text=~/understand/i) && defined($qglobals{ikky}) && ($qglobals{ikky} >= 4)) {
    plugin::DiaWind("Good, be on your way then. Return to me when you have completed your objective. I wish you luck and await your return.");
  }
  if (($text=~/pit of the lost/i) && defined($qglobals{ikky}) && ($qglobals{ikky} >= 4)) {
    plugin::DiaWind("I'm glad you've decided to continue helping the brotherhood. We can always use adventurers like you. There have been some mysterious happenings coming from the Pit of the Lost. I don't have the information on what's going on, but another of the brotherhood nearby does. You must venture south of the Martyrs Passage and speak with Tublik Narwether. He will know what's going on.");
  }
  if (($text=~/crumbled sanctuary/i) && defined($qglobals{ikky}) && ($qglobals{ikky} >= 4)) {
    plugin::DiaWind("I'm glad you're sticking around to help out with some additional tasks. We can certainly use the help. On the east side of the largest temple to the west, there exists a crumbled temple. As of yet, we're unsure what the cause or reason of the destruction was. I don't have all the information, but Tublik Narwether does. Seek him out to the south of the Martyrs Passage and find out what you can from him.");
  }
  if ((($text=~/temple of the damned/i) || ($text=~/discover their plan/i)) && defined($qglobals{ikky}) && ($qglobals{ikky} == 5)) {
    plugin::DiaWind("You must go into the Temple of the Damned. It is the temple directly behind me with the fog looming around it. Explore the temple and search for the instruments of power that the Legion of Mata Muram priests are collecting. Once you have the relics in hand, return them to me so I can study them further. Good luck to you, $name.");
  ...
  if (($text=~/stop the ritual/i) && defined($qglobals{ikky}) && ($qglobals{ikky} == 7)) {
    plugin::DiaWind("The Summoner's Ring is just past the Temple of the Damned");
  ...
  if (($text=~/have done all you asked/i) && defined($qglobals{ikkyalt}) && ($qglobals{ikkyalt} == 8)) {
    plugin::DiaWind("Indeed you have and your help is greatly appreciated. Take this back to L`diava I believe it has the answers she seeks.");
...
sub EVENT_ITEM {
  if (defined($qglobals{ikky}) && ($qglobals{ikky} == 4)) {
    ...
    if (defined($qglobals{ikky_flesh}) && ($qglobals{ikky_flesh} == 1) && plugin::check_handin(\%itemcount, 60145=> 1)) {
      plugin::DiaWind("Aha! I suspected there would be some kind of indication of what they were doing in that passage. I applaud your efforts. I only briefly skimmed the information and from what I can gather, it appears that there are nefarious deeds afoot. I'll need more time to examine this, but I should know what it says in a few moments. Well done, once again, $name!"); 
  ...
  if (defined($qglobals{ikky}) && ($qglobals{ikky} >= 7)) {
    if (plugin::check_handin(\%itemcount, 60151 => 1)) {
      plugin::DiaWind("You have done well, $name. Speak with Tublik to continue helping us here in Kod'Taz.");
