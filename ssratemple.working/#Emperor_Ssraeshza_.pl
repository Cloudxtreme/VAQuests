...
sub EVENT_SPAWN {
  $engaged = 0;
  quest::depop_with_timer(162065); # Removes the fake Emperor
...
sub EVENT_TIMER {
  if ($timer eq "EmpDepop") {
    ...
    if ($engaged == 0) {
      quest::depop();
      quest::signalwith(10,7,0); # Let zone_controller know we failed
    }
  }
}
...
sub EVENT_DEATH_COMPLETE {
  ...
  quest::signalwith(10,5,0); # Tell zone_controller Emp is dead
