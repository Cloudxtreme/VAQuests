...
sub EVENT_ITEM{
  if ($faction<=2) {  # need to be warmly or better
    if (plugin::check_handin(\%itemcount,5728=>1)) { # Di'zok Signet of Service
      ...
      plugin::DiaWind("Good! They tell'd me to wait here for ya, can't say I like it but here I sits. It be good to be rid of Skargus though, so I be a happy one, oh yes. Want to know the [plan] then?");
