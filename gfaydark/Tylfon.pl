...
sub EVENT_SAY {
   if ($text=~/hail/i) {
      if ($faction <= 7) {
         plugin::DiaWind("So, you think you have what it takes to be a Scout of Tunare? Come back with two gold and two rusty daggers and I'll make it worth your while.");
      }
      else {
         plugin::DiaWind("You dare to speak with a loyal member of the Scouts of Tunare?!! You are truly foolish!! Run away, while you still can.");
      }
   }
   elsif ($text=~/trades/i) {
      if ($faction <= 7) {
      ...
      if ($faction <= 7) {
         plugin::DiaWind("Seek out the Fier'Dal, Elmion. He was last heard telling the local bar patrons that he was off to do some adventuring at the lake near the estate. What that is, I do not know.");
      }
      else {
         plugin::DiaWind("You dare to speak with a loyal member of the Scouts of Tunare?!! You are truly foolish!! Run away, while you still can.");
      }
...
sub EVENT_ITEM {
   if (plugin::check_handin(\%itemcount, 18784 => 1)) { #Tattered Note
      ...
      plugin::DiaWind("Very well then, I thank you for assisting us. You have shown great bravery in aiding in this mission. Keep this as a symbol of our friendship.");
      quest::summonitem(54022);
      quest::exp(100); 
   }
   elsif (($gold >= 2) && (plugin::check_handin(\%itemcount, 7007 => 1))) {
      plugin::DiaWind("Well, well. I didn't think you could do it. Here's your cut and don't be surprised that it's not much because it's your first lesson. Remember. the smaller the operation. the bigger the share, and the richest rogues have the tightest lips.");
      ...
      plugin::DiaWind("Excellent job, $name, we will turn you into a rogue of Tunare yet.  Here this is for your trouble.");
